...
public class GoodsController {
    ...
    public String findByName(HttpServletRequest request, Model model,@RequestParam(required = false,defaultValue = "1") int count,
                             @RequestParam String name) {
        try {
            Map map=new HashMap();
            map.put("start",(count-1)*9);
            map.put("end",9);
            map.put("name",name);
            List<ShopInformation> shopInformations = shopInformationService.selectTen(map);
            UserInformation userInformation = (UserInformation) request.getSession().getAttribute("userInformation");
            if (StringUtils.getInstance().isNullOrEmpty(userInformation)) {
            ...
            int countsum=shopInformationService.getCountsByname(name);
            int page= countsum / 9 + ( countsum % 9 !=0 ? 1 : 0 );
    ...
    public String requireMall(HttpServletRequest request, Model model) {
        ...
        for (UserWant userWant : userWants) {
            ...
            //求购买家信息
            UserInformation userInformation1=userInformationService.selectByPrimaryKey(userWant.getUid());
            u.setUname(userInformation1.getUsername());
            list.add(u);
        }
    ...
    //通过分类选择求购商品
    @RequestMapping(value = "/selectRequireBySort.do")
    @ResponseBody
    public List<UserWant> selectRequireBySort(@RequestParam int sort) {
        return userWantService.selectBySort(sort);
    }
