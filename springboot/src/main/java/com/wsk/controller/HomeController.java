...
public class HomeController {
    ...
    public String mallPage(HttpServletRequest request, Model model,@RequestParam(required = false,defaultValue = "1") int count,@RequestParam(required = false,defaultValue = "999") int sort) {
        UserInformation userInformation = (UserInformation) request.getSession().getAttribute("userInformation");
        if (StringUtils.getInstance().isNullOrEmpty(userInformation)) {
            userInformation = new UserInformation();
        }
        model.addAttribute("userInformation", userInformation);
        try {
            List<ShopInformation> shopInformations=null;
            if(sort==999){
                shopInformations = selectTen(count, 9);
            }else {
                shopInformations = selectTenBySort(count, 9, sort);
            }
            List<ShopInformationBean> list = new ArrayList<>();
            int counts = getShopCounts();
            model.addAttribute("shopInformationCounts", counts);
            String sortName;
            for (ShopInformation shopInformation : shopInformations) {
                sortName = getSortName(shopInformation.getSort());
                ShopInformationBean shopInformationBean = new ShopInformationBean();
                shopInformationBean.setId(shopInformation.getId());
                shopInformationBean.setName(shopInformation.getName());
                shopInformationBean.setLevel(shopInformation.getLevel());
            ...
            int countsum;
            if(sort==999){
                countsum=shopInformationService.getCounts();
            }else{
                countsum=shopInformationService.getCountsBysort(sort);
            }
...
//        System.out.println(sort);
    ...
    //获取商品，分页,一次性获取end个
    private List<ShopInformation> selectTenBySort(int start, int end,int sort) {
        Map map = new HashMap();
        map.put("start", (start - 1) * end);
        map.put("end", end);
        map.put("sort",sort);
