...
public class UserController {
    ...
    //从收藏查看用户基本信息
    @RequestMapping(value = "/watch_personal_info.do")
    public String watchpersonalInfo(HttpServletRequest request, Model model,@RequestParam int id) {
        UserInformation userInformation= userInformationService.selectByPrimaryKey(id);
        model.addAttribute("userInformation", userInformation);
        return "page/watch_personal_info";
    }
    ...
    public Map certification(HttpServletRequest request,
                             ...
                             @RequestParam(required = false) String contact, @RequestParam String token,
...
//        map.put("result", result);
    ...
    //修改我的求购商品
    @RequestMapping(value = "/reviseUserWant.do")
    public String reviseUserWant(HttpServletRequest request, Model model,
...
//        userCollection.setSid(sid);
    ...
    public String selectShopCar1(HttpServletRequest request, Model model) {
        UserInformation userInformation = (UserInformation) request.getSession().getAttribute("userInformation");
        if (StringUtils.getInstance().isNullOrEmpty(userInformation)) {
        ...
        for (GoodsCar goodsCar : goodsCars) {
            ...
            goodsCarBean.setQuantity(shopInformation.getQuantity());
            //商家信息
            UserInformation userInformation1=userInformationService.selectByPrimaryKey(shopInformation.getUid());
            goodsCarBean.setSname(userInformation1.getUsername());
            goodsCarBean.setSuid(shopInformation.getUid());
...
//    }
    ...
    @ResponseBody
    public BaseResponse insertGoodsCar(HttpServletRequest request, @RequestParam int sid) {
        UserInformation userInformation = (UserInformation) request.getSession().getAttribute("userInformation");
        if (StringUtils.getInstance().isNullOrEmpty(userInformation)) {
        ...
        goodsCar.setUid(uid);
        goodsCar.setSid(sid);
        if(goodsCarService.selectByPrimaryKey(goodsCar)!=null){
    ...
    @ResponseBody
    public BaseResponse deleteShopCar(HttpServletRequest request, @RequestParam int sid) {
        UserInformation userInformation = (UserInformation) request.getSession().getAttribute("userInformation");
        if (StringUtils.getInstance().isNullOrEmpty(userInformation)) {
            return BaseResponse.fail(2);
        }
        ...
        goodsCar.setSid(sid);
        goodsCar.setUid(uid);
        int result = goodsCarService.deleteByPrimaryKey(goodsCar);
        if (result != 1) {
            return BaseResponse.fail();
        }
        return BaseResponse.success();
    ...
    //批量删除购物车的商品
    @RequestMapping(value = "/deleteShopCarBySids.do")
    @ResponseBody
    public BaseResponse deleteShopCarBySids(HttpServletRequest request, @RequestParam(value = "sids[]") Integer[] sids) {
        UserInformation userInformation = (UserInformation) request.getSession().getAttribute("userInformation");
        if (StringUtils.getInstance().isNullOrEmpty(userInformation)) {
            return BaseResponse.fail(2);
        }
        ...
        for(int sid:sids){
            ...
            goodsCar.setSid(sid);
            goodsCar.setUid(uid);
            goodsCarService.deleteByPrimaryKey(goodsCar);
        }
        return BaseResponse.success();
    }
