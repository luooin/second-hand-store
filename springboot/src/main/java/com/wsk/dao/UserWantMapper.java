...
public interface UserWantMapper {
    ...
    @Select("select * from userwant where uid=#{id} and display=1 order by id desc")
    List<UserWant> selectMineByUid(int id);

    List<UserWant> selectAll();

    @Select("select * from userwant where sort=#{sort} and display=1 order by id desc limit 9")
